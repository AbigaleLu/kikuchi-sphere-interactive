/* 原生 Canvas：每面一张 PDF 裁图，正交投影，不计算衍射关系。 */
(() => {
  'use strict';
  ...
  if (!ctx || !data) {
    $('loading').textContent = '模型资料未能载入。请保留整个项目文件夹，再打开 index.html。';
    return;
  }
  ...
  let q = home(), model = 'FCC', zoom = 1, selected = null, highlight = false;
  ...
  function draw(time) {
    ...
    if (ready) for (const item of visible) {
      ...
      ctx.drawImage(textures[model][i],0,0);
      ctx.restore();
      ctx.beginPath();poly.forEach((p,k) => k ? ctx.lineTo(...p) : ctx.moveTo(...p));ctx.closePath();
  ...
  function selectFace(index,mark=false) {
    ...
    $('face-preview').src=face.images[model];
    $('face-preview').alt=`${model} ${face.label} 面的 PDF 原图，保留原有线条、标签与色彩`;
  ...
  function setModel(next) {
    ...
    $('model-description').textContent=`${name} · PDF 第 ${model==='FCC'?1:2} 页`;
  ...
  function uprightFront() {
    const right=rotate(data.faces[frontFace()].u);
