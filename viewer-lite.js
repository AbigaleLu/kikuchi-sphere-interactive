...
(() => {
  ...
  function currentViewUrl(page) {
    const url=new URL(location.protocol==='file:'?'https://abigalelu.github.io/kikuchi-sphere-interactive/index-lite.html':location.href);url.hash='';url.search='';
  ...
  function release(e) {
    ...
    if (e.type==='pointerup' && !dragged && !pinched && ready) {
      ...
      if (hit) {clearTimeout(settleTimer);selectFace(hit.i,true);}
    }
